...
public class PurchaseBalDueViewModel extends MainViewModel {

    MutableLiveData<CommanResorce<List<BalanceDueResponse>>> _data;

    public PurchaseBalDueViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<BalanceDueResponse>>> getPuchaseBalanceDueData(String FromDate,String ToDate,String SlpName,String DbName,String Flag){
        CommanResorce<List<BalanceDueResponse>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();
        repository.getPurchaseDueDetails( FromDate, ToDate, SlpName, DbName, Flag, new Callback<List<BalanceDueResponse>>() {
            @Override
