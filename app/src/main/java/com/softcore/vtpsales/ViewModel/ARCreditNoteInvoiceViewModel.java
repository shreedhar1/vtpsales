...
public class ARCreditNoteInvoiceViewModel extends MainViewModel {

    MutableLiveData<CommanResorce<List<ARInvoiceModel>>> _data;

    public ARCreditNoteInvoiceViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<ARInvoiceModel>>> getArCNInvoiceData(String DbName){
        CommanResorce<List<ARInvoiceModel>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();

        repository.getArCreditNoteInvDet(DbName, new Callback<List<ARInvoiceModel>>() {
            @Override
