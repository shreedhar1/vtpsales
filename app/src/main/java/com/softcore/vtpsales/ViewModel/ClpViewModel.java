...
public class ClpViewModel extends MainViewModel{

    MutableLiveData<CommanResorce<List<SlpResponse>>> _data;

    public ClpViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<SlpResponse>>> getClpData(){
        CommanResorce<List<SlpResponse>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();
        repository.getClpDetails( new Callback<List<SlpResponse>>() {
            @Override
