...
public class MonthWiseReportViewModel extends MainViewModel {

    MutableLiveData<CommanResorce<List<CusReportWiseModel>>> _data;

    public MonthWiseReportViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<CusReportWiseModel>>> getMonthsWiseReportData(String FromDate, String ToDate, String SlpName, String DbName, String Flag){
        CommanResorce<List<CusReportWiseModel>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();

        repository.getMonthWiseReportDetails( FromDate, ToDate, SlpName, DbName, Flag, new Callback<List<CusReportWiseModel>>() {
            @Override
