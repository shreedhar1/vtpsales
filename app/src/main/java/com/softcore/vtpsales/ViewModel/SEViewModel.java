...
public class SEViewModel extends MainViewModel{

    MutableLiveData<CommanResorce<List<SlpResponse>>> _data;

    public SEViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<SlpResponse>>> getSEData(){
        CommanResorce<List<SlpResponse>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();
        repository.getSEDetails( new Callback<List<SlpResponse>>() {
            @Override
