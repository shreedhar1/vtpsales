...
public class APCreditNoteInvoiceViewModel extends MainViewModel {

    MutableLiveData<CommanResorce<List<ARInvoiceModel>>> _data;

    public APCreditNoteInvoiceViewModel(@NonNull Application application) {
        super(application);
        _data=new MutableLiveData<>();

    }
    public LiveData<CommanResorce<List<ARInvoiceModel>>> getApCNInvoiceData(String DbName){
        CommanResorce<List<ARInvoiceModel>> _new_data=new CommanResorce<>();
        _data=new MutableLiveData<>();

        repository.getApCreditNoteInvDet(DbName, new Callback<List<ARInvoiceModel>>() {
            @Override
